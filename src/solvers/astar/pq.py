...
The priority queue is stored as a list wherevposition 0 always contains None.
The first actual item is stored in position 1.  This is necessary so that the 
list can be treated as a binary tree and simple calculations can be done to find the
...
class PriorityQueue:
    ...
    def min(self):
        if self.empty():
            print("Error finding min in queue", file=sys.stderr)
            raise RuntimeError
        return self.q[1]
